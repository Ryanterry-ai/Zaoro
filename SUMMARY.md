...
## Generic extraction + noise→silence scroll-accumulation (Tasks A/B/C)
- **Goal:** any prompt (e.g. "futuristic headphone website, every scroll transforms noise into silence") yields an unforgettable, signal-derived experience with NO industry table, NO per-vertical template, NO renderer business reasoning.
- **A — Generic entity extraction (no industry entity defaults):**
  - `business-intelligence/dimensions.ts` — `extractDomainNouns` rewritten to be GENERIC: linguistic salience (stopword-filtered, plural→singular), excludes `GENERIC_CATEGORY_WORDS` only; captures ANY novel product noun (`headphone`, `soundwave`, `skincare`, …) without a curated list. `LEXICON_SURFACE` hoisted above the function.
  - `business-intelligence/archetypes.ts` — `composeDiscovery` now builds `businessType` from PRIMITIVES ONLY (quality + shape base); the user's raw domain noun is NO LONGER injected into the type label (a generic word like "team" must never reshape classification). The noun still flows into `vocabulary.domainNouns` + `composeIntent`.
  - `deterministic-orchestrator-v4.ts` — after overlaying `businessKnowledge`, also sets `breContext.entities` from `businessKnowledge.entities` (generic `Product`, not `['User']`).
  - Result: "headphone website" → `product-nature=physical-good` → `sell-products` goal → `Product` entity generically.
- **B — Motion-intent derivation from description:**
  - `experience-intelligence/experience-engine.ts` — `ExperienceEngineInput` gains `description?`; new `deriveMotionIntent(description)` returns `{ scrollDriven, transformArc:{from,to}, theme }` via `SCROLL_ANCHORS` + `TRANSFORM_PAIRS` (noise→silence, chaos→calm, clutter→clarity, tension→ease, static→flow, noise→peace). Generic, no industry.
  - `generateExperienceBlueprint` calls `deriveMotionIntent(description)`; enables `scrollAccumulation` when `style==='cinematic'||'luxury'|| (scrollDriven && transformArc)`; `metric`/`label` use `theme`; carries optional `transformArc` on config.
  - `experience-intelligence/types.ts` — `ScrollAccumulationConfig.transformArc?: {from;to}`.
  - `pipeline/canonical-build.ts` — `expInput.description = opts.prompt`.
  - `generation/build-pipeline.ts` — Layer 3b passes `description` to `generateExperienceBlueprint` so the renderer's own blueprint derives the arc from the prompt.
- **C — Generic scroll-transform renderer:**
  - `generation/renderers/react-renderer.ts` — `renderScrollIntensity` embeds `transformArc`: adds an ambient narrative crossfade (chaotic `from` → calm `to` via `useTransform` opacity/blur) when an arc is present; meter `label`/`metric` already generic ("Silence" for noise→silence). `ScrollIntensity` wired into `layout.tsx`.
- **Tests:** `tests/scroll-transform-arc.test.ts` (NEW, 3) asserts headphone prompt → `Product` entity + `scrollAccumulation.enabled` + `transformArc:{from:'noise',to:'silence'}`. `tests/business-intelligence.test.ts` (104) updated: domain nouns are now generic (captures `coffee`/`headphone`, excludes stopwords) — proves no hardcoded dictionary.
- **Status:** typecheck clean; full suite GREEN: 75 files / 1053 tests. The "futuristic headphone website, scroll transforms noise into silence" prompt now produces a scroll-accumulation experience end-to-end with no industry branching.
