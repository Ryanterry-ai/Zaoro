...
function makeOutcome(overrides?: Partial<Parameters<ExperienceEngine['recordOutcome']>[0]>) {
  return {
    ...
    stages: [
      { stageId: 'research', provider: 'groq', model: 'llama-3.3-70b-versatile', taskType: 'analysis' as const, durationMs: 5000, tokensUsed: 2000, llmCalls: 1, success: true, error: undefined, cacheHit: false },
    ],
...
describe('ExperienceEngine', () => {
  ...
  describe('Recommendations', () => {
    it('should return low-confidence defaults when no history', () => {
      ...
      const recs = engine.getRecommendations({ stageId: 'research', taskType: 'analysis', industry: 'saas' });
  ...
  describe('History Queries', () => {
    ...
    it('should filter by stage', () => {
      ...
      engine.recordOutcome(makeOutcome({ stages: [{ stageId: 'research', provider: 'groq', model: 'm', taskType: 'analysis', durationMs: 1, tokensUsed: 1, llmCalls: 1, success: true, error: undefined, cacheHit: false }] }));
      engine.recordOutcome(makeOutcome({ stages: [{ stageId: 'architecture', provider: 'anthropic', model: 'm', taskType: 'planning', durationMs: 1, tokensUsed: 1, llmCalls: 1, success: true, error: undefined, cacheHit: false }] }));
  ...
  describe('Statistics', () => {
    ...
    it('should compute by-stage stats', () => {
      ...
      engine.recordOutcome(makeOutcome({ stages: [{ stageId: 'research', provider: 'groq', model: 'm', taskType: 'analysis', durationMs: 5000, tokensUsed: 2000, llmCalls: 1, success: true, error: undefined, cacheHit: false }] }));
      const stats = engine.getStats();
  ...
  describe('Stage Performance', () => {
    it('should compute stage stats with errors', () => {
      ...
      engine.recordOutcome(makeOutcome({ stages: [{ stageId: 'research', provider: 'groq', model: 'm', taskType: 'analysis', durationMs: 5000, tokensUsed: 2000, llmCalls: 1, success: false, error: 'timeout', cacheHit: false }] }));
